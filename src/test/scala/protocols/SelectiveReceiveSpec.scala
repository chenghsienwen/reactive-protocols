...
/*
sbt 'testOnly protocols.ProtocolsSuite -- -z "A SelectiveReceive Decorator must overflow (size 0)"'
* */
trait SelectiveReceiveSpec extends FunSuite with PropertyChecks with MustMatchers {
    ...
    def expectOne[T](inbox: TestInbox[T], seq: List[T]): Behavior[T] = {
      seq match {
        case x :: xs =>
          //println(s"get seq ${seq.mkString(",")}")
          receiveMessagePartial {
            case `x` =>
              //println(s"receiveMessagePartial $x")
              inbox.ref ! x
              expectOne(inbox, xs)
          }
